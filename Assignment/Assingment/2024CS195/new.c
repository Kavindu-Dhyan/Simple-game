...
/*void initialize_players(Player players[NUM_PLAYERS]) {
    for (int i = 0; i < NUM_PLAYERS; i++) {
        players[i].f = 0;                  // Start at ground floor
        players[i].w = players[i].first_w; // Starting width coordinate
        players[i].l = players[i].first_l; // Starting length coordinate
        players[i].movement_points = 100;  // Full movement points
        players[i].in_maze = 0;            // Not yet in the maze
        players[i].disoriented_turns = 0;
        players[i].triggered_turns = 0;
        players[i].food_poisoned_turns = 0;
        players[i].throws = 0;

        // Place player on grid if needed
...
void initialize_players(Player players[NUM_PLAYERS]) {
    // Example starting positions for 3 players
    int start_w[NUM_PLAYERS] = {0, 0, 0}; // adjust as needed
    int start_l[NUM_PLAYERS] = {5, 9, 12}; // adjust as needed
    char names[NUM_PLAYERS] = {'A', 'B', 'C'};

    for (int i = 0; i < NUM_PLAYERS; i++) {
        players[i].name = names[i];
        players[i].f = 0;                // ground floor
        players[i].w = start_w[i];
        players[i].l = start_l[i];
        players[i].first_w = start_w[i];
        players[i].first_l = start_l[i];
        players[i].movement_points = 100;
        players[i].in_maze = 0;          // not in maze yet
        players[i].throws = 0;
        players[i].disoriented_turns = 0;
        players[i].triggered_turns = 0;
        players[i].food_poisoned_turns = 0;
        // initialize any other Player struct fields here
    }
}
...
void clamp_floor(Player *p) {
    if (p->f < 0) p->f = 0;
    if (p->f >= FLOORS) p->f = FLOORS - 1;
}
...
int check_stair(Player *p) {
    for (int i=0; i<stair_count; i++) {
        ...
        if (p->f==s.sf && p->w==s.sw && p->l==s.sl) {
            ...
            clamp_floor(p);  // added clamp here
            return 1;
...
void move_player(Player *p, int steps, Player players[NUM_PLAYERS]) {
    ...
    clamp_floor(p);     // Safety: make sure floor is valid at the very start
    ...
    if (can_move(p, effective_steps, move_dir)) {
        ...
        for (int i = 0; i < effective_steps; ++i) {
            ...
            check_stair(p);
            check_pole(p);
            clamp_floor(p); // added clamp here after stairs/poles
        ...
        clamp_floor(p); // final clamp after movement
...
void player_turn(Player *p, Player players[NUM_PLAYERS]){
    ...
    if(can_move(p, move, p->dir)){
        ...
        // Deduct the 2 movement points for unsuccessful move
        p->movement_points -=2;

        if(p->movement_points <= 0){
            printf("%c movement points are depleted and requires replenishment. Transporting to Bawana.\n", p->name);

            // --- FIX: Use transport_to_bawana to properly place player ---
            transport_to_bawana(p, players);

            int total_cost = 2; // dice cost
...
int main(void) {
    ...
    /*----------Players----------*/
    Player A = {'A', 0, 0, 11, 5, 11, NORTH,0,0,100};
    Player B = {'B', 0, 0, 7, 9, 6, WEST,0,0,100};
    Player C = {'C', 0, 0, 15, 9, 16, EAST,0,0,100};
